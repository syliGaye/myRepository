...
if(!$libelle){
    ...
    if($nbreHeure === '0'){
        ...
        if(count($dn) > 0){$message = 'Type de Session existant.';}
        else{
            $i = 0 ;
            ...
            if(count($dn0) > 0){$message = 'Type de Session existant.';}
            else{
                $typeSession->saveTypeSession($db, $libelle, $jourHeure, $nbreHeure);
                $message = 'ok';
            }
        }
    }
